...
Summary: Live Intelligence (FR-7, US-806..814) implemented trên branch `feat/correction-library` — xem Active Feature. Điểm cần nhớ khi resume: (1) mlx revise trả None cho tới khi mlx-whisper có beam search — nếu nâng mlx-whisper, bỏ comment check trong `MlxEngine.revise` là tự chạy; (2) prompt ASR giờ xếp user glossary CUỐI (khác bản pass 2 user-first) — đừng "sửa" lại tưởng là bug; (3) speaker-ID live cần voiceprint đã enroll + models diarize, không thì tự tắt; (4) ĐÃ merge vào main 2026-07-20 (`ed4b25b`) theo yêu cầu user — main giờ chứa speaker-layer + correction-library + live-intelligence; các branch feat/* giữ lại làm mốc lịch sử. Đợt 2 (org sync) vẫn dở — xem mục bên dưới.
