...
| US-814 Speaker-ID | utterance final ≥1.5s → `ident_q` (max 3) → `diarize.embed_utterance` + `best_match` (thread riêng, không đụng decode lock) → WS `{type:"speaker"}`; ĐỔI người → bias xếp term người đó lên đầu (đổi người = debounce). Tắt im lặng nếu thiếu model/voiceprint. Env `LIVE_ID_THRESHOLD`. **Fix 2026-07-20**: opt-in setting `live_ident` default OFF — embedding ONNX mỗi câu tranh CPU/RAM với decode làm tick chậm, cắt câu lệch (regression thực địa); UI toggle chưa có, bật qua PUT /api/settings (tech-debt) | diarize.py, live.py, app.js |
